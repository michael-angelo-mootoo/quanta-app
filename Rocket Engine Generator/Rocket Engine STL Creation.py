...
def export_3D(contour, filename="nozzle.stl", file_format="stl"):
    x = np.concatenate([contour[0], contour[3], contour[6]])  # xe, xe2, xbell
    y = np.concatenate([contour[1], contour[4], contour[7]])  # ye, ye2, ybell
    z = np.zeros_like(x)  # Initial z-plane

    # Generate rotation for 3D revolution
    ...
    for i, theta in enumerate(angles):
        ...
        for j in range(len(x)):
            verts.append([x[j], y[j] * cos_t, y[j] * sin_t])  # Rotating in 3D
    ...
    """
    os.rename("C:/Users/Public/Operational_Functional_Block/Python_Projects/Rocket Engine Generator/nozzle_stl",
              "C:/Users/Public/Operational_Functional_Block/Python_Projects/Rocket Engine Generator/nozzle_stl.stl")
    """


def export_STEP(contour, filename="nozzle.step"):
    x = np.concatenate([contour[0], contour[3], contour[6]])
    y = np.concatenate([contour[1], contour[4], contour[7]])

    shape = cq.Workplane("XZ").spline(list(zip(x, y))).revolve(360)
    shape.val().exportStep(filename)
    """
    os.rename("C:/Users/Public/Operational_Functional_Block/Python_Projects/Rocket Engine Generator/nozzle_step",
              "C:/Users/Public/Operational_Functional_Block/Python_Projects/Rocket Engine Generator/nozzle_step.stl")
    """
...
if __name__ == "__main__":
    ...
    aratio = 20  # Ae / At
    throat_radius = 7.5  # {'radius_throat': 40, 'radius_exit': 210}
    ...
    export_3D(contour, filename="nozzle.stl", file_format="stl")